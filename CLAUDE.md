...
- **The principal artist carries the requirement.** `score_artist` scores the principal and adds only `GUEST_BONUS` per matched guest, so a perfect featured-artist match can never stand in for a missing principal (`Jay-Z feat. Alicia Keys` against a result credited to Alicia Keys alone scores ~0.05). `with` is a separator in *artist* names only — in a title it's an ordinary word, and splitting on it reduced `With or Without You` to nothing.
