...
Cratefill is a single-file Tkinter desktop app (`cratefill.py`) that loads songs from a CSV (artist + title) and adds them to the user's YouTube Music playlists via the unofficial [ytmusicapi](https://github.com/sigma67/ytmusicapi) library. See `implementation-notes.md` for the full architecture walkthrough and design rationale; `RESEARCH.md` for why ytmusicapi was chosen over the official YouTube Data API.
...
```powershell
py -m pip install -r requirements.txt   # install deps (just ytmusicapi)
...
`read_songs_csv` and `pick_match` are pure functions — test them directly (e.g. against `sample.csv`). Anything hitting YouTube Music (search, playlists, adding) requires a real logged-in session in `browser.json` and can only be tested manually.
...
- **Threading rule:** network I/O for the add operation runs in a worker thread (`_worker`) that communicates only via `self.worker_queue`, drained on the main thread by `_poll_worker` (`root.after` loop). Never touch a Tk widget from the worker thread.
...
- Searches are one call per song; playlist adds are batched (one `add_playlist_items` call per playlist with all videoIds), with `duplicates=False`.
