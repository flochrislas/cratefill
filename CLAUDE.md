...
- **Coming back empty-handed is the worst outcome.** `matching.choose_match` returns a `MatchDecision` with status `high` / `ambiguous` / `rejected`, and `rejected` means **a different song** — specifically, no result shared a whole word with the requested title (`title_score > TITLE_FLOOR`). A remix, a live take, a karaoke version or another band's cover of the right song is *offered* as `ambiguous`, because a reviewable proposal beats a silent miss. There is still no first-result fallback: `one → someone` and `cher → cherub` share no whole word and stay rejected, and so does another track by the right artist.
- **A wrong artist or wrong version costs points, it doesn't exclude.** Both block the `high` tier and are named in `reasons`, and the version difference is folded into the score through `VERSION_PENALTY` (`same` 0, `missing` 0.10, `extra` 0.20) rather than filtering. That ranking matters: the real artist's live take must beat a tribute band's studio cut, which a hard version filter got backwards. `version_relation` is asymmetric — `extra` (a marker you didn't ask for) is penalised harder than `missing` (only the standard recording exists).
- `core_title` drops a bracketed group **whole** when it names a version, so `Wonderwall (Live at Wembley)` still scores 1.00 on title; keeping `at wembley` made the live take look like a different song. Soft markers (remastered, deluxe, album version…) and `feat. X` go the same way. Comparison is on whole normalized tokens, never substrings. Thresholds are module constants at the top of `matching.py` — tune there, with tests.
